...
# The task is to implement the missing funcion camcalibDLT.py.
# The algorithm is summarised on the exercise sheet.
...
for i in range(8):
    plt.plot(pproj1[0,i] / pproj1[2,i], pproj1[1,i] / pproj1[2,i], 'rx', markersize=12)

# Remove ticks and show
plt.xticks([])
plt.yticks([])
plt.show()
...
for i in range(len(x1)):    
    plt.annotate(ascii_lowercase[i], (x2[i], y2[i]), color='c', fontsize=20)


## Your task is to implement camcalibDLT.py
...
for i in range(8):
    plt.plot(pproj2[0,i] / pproj2[2,i], pproj2[1,i] / pproj2[2,i], 'rx', markersize=12)
