...
# Function for drawing eplines within image borders
def draw_eplines(eplines, im1, im2, style='c-'):
    ...
    for i in range(np.shape(eplines)[1]):
        ...
        if (-a*np.shape(im2)[1] - c) / b < 0:
            yc2 = 0
            xc2 = -c/a
        elif(-a*np.shape(im2)[1] - c) / b > np.shape(im2)[0]:
            yc2 = np.shape(im2)[0]
            xc2 = (-b*yc2-c)/a
        else:
            xc2 = np.shape(im2)[1]
